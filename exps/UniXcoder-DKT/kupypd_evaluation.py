...
def performance_granular(batch, pred, ground_truth, prediction, n_problems, entire_maxstep):
    preds = {k:[] for k in range(n_problems)}
    gts = {k:[] for k in range(n_problems)}
    first_preds = {k:[] for k in range(n_problems)}
    first_gts = {k:[] for k in range(n_problems)}
    scores = {}
    first_scores = {}
    
    for s in range(pred.shape[0]):
        delta = (batch[s][1:, 0:n_problems] + batch[s][1:, n_problems:n_problems*2])
        temp = pred[s][:entire_maxstep-1].mm(delta.T)
        index = torch.tensor([[i for i in range(entire_maxstep-1)]],
                             dtype=torch.long)
        p = temp.gather(0, index)[0].detach().cpu().numpy()
        a = (((batch[s][:, 0:n_problems] - batch[s][:, n_problems:n_problems*2]).sum(1) + 1) // 2)[1:].detach().cpu().numpy()
    ...
    for j in range(n_problems):
        ...
        # print(f"problem {j}. auc: {auc}. f1: {f1}. recall: {recall}. precision: {precision}. acc: {acc}")
        ...
        # print(f"First prediction for problem {j}. auc: {first_auc}. f1: {first_f1}. recall: {first_recall}. precision: {first_precision}. acc: {first_acc}")
...
def plot_heatmap(batch, pred, fold, batch_n, n_problems):
    ...
    for s in range(pred.shape[0]):
        ...
        delta = (batch[s][1:, 0:n_problems] + batch[s][1:, n_problems:n_problems*2]).detach().cpu().numpy()
        
        a = (((batch[s][:, 0:n_problems] - batch[s][:, n_problems:n_problems*2]) + 1) // 2)[1:].detach().cpu().numpy()
        ...
        plt.xticks(np.arange(len(problems))+0.5, n_problems, rotation=45)
        plt.yticks(np.arange(n_problems)+0.5, ['234', '13', '232', '233', '5', '235', '236', '1', '3'], rotation=45)
...
class lossFunc(nn.Module):
    def __init__(self, num_of_questions, entire_maxstep, device):
        super(lossFunc, self).__init__()
        self.crossEntropy = nn.BCELoss()
        self.num_of_questions = num_of_questions
        self.entire_maxstep = entire_maxstep
        self.device = device
    ...
    def forward(self, pred, batch):
        ...
        for student in range(pred.shape[0]):
            ...
            delta = batch[student][:, 0:self.num_of_questions] + batch[student][:, self.num_of_questions:self.num_of_questions*2]  # shape: [length, questions]
            temp = pred[student][:self.entire_maxstep-1].mm(delta[1:].t())
            index = torch.tensor([[i for i in range(self.entire_maxstep-1)]], dtype=torch.long)
...
def test_epoch(model, testLoader, loss_func, device, n_problems, entire_maxstep):
    model.to(device)
    ground_truth = torch.tensor([])
    prediction = torch.tensor([])
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(testLoader, desc='Testing:     ', mininterval=2):
            batch_new = batch[:,:-1,:].to(device)   # t_0 ~ t_{n-1} to predict t_n
            pred = model(batch_new)
    ...
    return performance_granular(full_data, preds, ground_truth, prediction, n_problems, entire_maxstep)
