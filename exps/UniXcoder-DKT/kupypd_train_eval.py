...
import json
def train(is_hp_search, config:kupypd_config.Config, semester_group, data_dir, cached_data:CacheDataLoad,
    ...
    from CodeEmbedRNNModel import CodeEmbedRNNModel
    ...
    for k, v in config.__dict__.items():
        if k == "solving_interactions":
            v = v.name
        config_dict[k] = v
    logger.info(f"config:"+json.dumps(config_dict, indent=3))

    cached_data.load_data(is_hp_search=is_hp_search, config=config, semester_group=semester_group, data_dir=data_dir)

    n_problems = cached_data.reader.numofques
    code_emb_size = cached_data.code_emb_size

    is_early_stop = is_hp_search   # when searching hp, stop early

    train_msg = f"{log_prefix}. problems: {n_problems}. epochs:{config.epochs}, bs:{config.batch_size}, code emb:{code_emb_size}, lr:{config.lr}, es:{is_early_stop}"
    ...
    for fold in range(folds):
        ...
        model = CodeEmbedRNNModel(device,
                                  config,
                                  n_problems * 2,
                                  n_problems,
                                  code_emb_size=code_emb_size) 
        ...
        if is_early_stop:
            ...
            fold_result_list.append(epoch_results[-1][1])
            logger.info(f"from 0 fold to now({fold} fold) , mean of auc: {np.mean(fold_result_list):.4f}")
    ...
    mean_auc = float(np.mean(fold_result_list))
    logger.info(f"{train_msg}. end. mean auc:{mean_auc:.4f} elapse:{time.time()-run_st}")
