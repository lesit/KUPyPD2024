...
import pandas as pd
def train_list(gpu_devices, is_hp_search, data_root_dir, train_info_list, data_aux_path, folds, logger, logger_name, logger_dir, debug):
    if debug or len(gpu_devices) < 2:
        if len(gpu_devices)>=1:
            os.environ["CUDA_VISIBLE_DEVICES"] = gpu_devices[0]
        cached_data = CacheDataLoad(data_root_dir, data_aux_path, logger, debug)
