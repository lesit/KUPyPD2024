...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    if model_name in ["atdkt", "simplekt", "stablekt", "bakt_time", "sparsekt", "cskt"]:
        ...
        if model.emb_type.find("predcurc") != -1:
            ...
            loss = loss1
    elif model_name in ["rekt", "rekt_concept"]:
...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, test_loader=None, test_window_loader=None, save_model=False, data_config=None, fold=None, logger=None):
    ...
    for i in range(1, num_epochs + 1):
        ...
        if logger is not None:
            logger.info(f"Epoch: {i}, validauc: {validauc:.4}, validacc: {validacc:.4}, best epoch: {best_epoch}, best auc: {max_auc:.4}, train loss: {loss_mean}, emb_type: {model.emb_type}, model: {model.model_name}, save_dir: {ckpt_path}")
            logger.info(f"            testauc: {round(testauc,4)}, testacc: {round(testacc,4)}, window_testauc: {round(window_testauc,4)}, window_testacc: {round(window_testacc,4)}")
        else:
