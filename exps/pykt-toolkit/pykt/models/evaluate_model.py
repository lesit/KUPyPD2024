...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            for key in dcurori.keys():
                dcurori[key] = dcurori[key].to(device)
            for key in dqtest.keys():
                dqtest[key] = dqtest[key].to(device)
    ...
    with torch.no_grad():
        ...
        for i, row in df.iterrows():
            ...
            if model_name == "lpkt":
                ...
                skills_difficult_path = dpath+f'/skills_difficult_{model.difficult_levels}.csv'
                questions_difficult_path =  dpath+f'/questions_difficult_{model.difficult_levels}.csv'
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["atdkt"]: ## need change!
           ...
           pred = y[0][-1]
        elif model_name in ["simplekt", "rekt", "rekt_concept", "stablekt", "sparsekt","cskt"]:
           if qout != None:
               curq = torch.tensor([[qout.item()]]).to(device)
               qinshft = torch.cat((qin[:,1:], curq), axis=1)
           else:
