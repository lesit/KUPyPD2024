...
from .rekt import ReKT, ReKT_concept
...
def init_model(model_name, model_config, data_config, emb_type):
    if model_name == "dkt":
        ...
        model = ReKT(data_config["num_c"], data_config["num_q"], **model_config).to(device)
    elif model_name == "rekt_concept":
        model = ReKT_concept(data_config["num_q"], data_config["num_c"], **model_config).to(device)
